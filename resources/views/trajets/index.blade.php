            ...
            @foreach ($trajets as $trajet)
                    ...
                    <div class="card-content">
                        ...
                        <div class="card-actions">
                            ...
                            @auth
                                ...
                                @else
                                    @if($trajet->places_disponibles > 0)
                                        <button class="action-btn action-btn-success" onclick="openReservationModal({{ $trajet->id }}, {{ $trajet->places_disponibles }})">
                ...
                <div id="modal-{{ $trajet->id }}" class="reservation-modal">
                    ...
                    <div class="modal-content">
                        <button class="modal-close" onclick="closeReservationModal({{ $trajet->id }})">&times;</button>
                        ...
                        <div class="modal-trip-info">
                            <div class="info-row">
                                ...
                                <span class="info-value">{{ $trajet->ville_depart }} → {{ $trajet->ville_arrivee }}</span>
                            </div>
                            <div class="info-row">
                            ...
                            <div class="info-row">
                                ...
                                <span class="info-value">{{ $trajet->conducteur->prenom }} {{ $trajet->conducteur->nom }}</span>
                            </div>
                        </div>
                        ...
                        <form action="{{ route('reservations.store', $trajet) }}" method="POST" class="modal-form">
                            ...
                            <div class="form-group">
                                <label for="nombre_places-{{ $trajet->id }}" class="form-label">Nombre de places:</label>
                                <div class="seat-selector">
                                    <button type="button" class="seat-btn" onclick="decrementSeats({{ $trajet->id }})">−</button>
                                    <input type="number" id="nombre_places-{{ $trajet->id }}" name="nombre_places" value="1" min="1" max="{{ $trajet->places_disponibles }}" class="seat-input" readonly>
                                    <button type="button" class="seat-btn" onclick="incrementSeats({{ $trajet->id }}, {{ $trajet->places_disponibles }})">+</button>
                                </div>
                                <p class="seat-hint">Sélectionnez le nombre de places à réserver (max: {{ $trajet->places_disponibles }})</p>
                            </div>

                            <div class="modal-actions">
                                <button type="button" class="btn btn-secondary" onclick="closeReservationModal({{ $trajet->id }})">
                                    Annuler
                                </button>
