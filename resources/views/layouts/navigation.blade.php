                ...
                <li class="dropdown-menu">
                    ...
                    <div class="dropdown-content">
                        ...
                        <form method="POST" action="{{ route('logout') }}" style="margin: 0;">
                            @csrf
                            <button type="submit"
                                style="width: 100%; text-align: left; background: none; border: none; padding: 1rem 1.5rem; cursor: pointer; color: var(--text-dark);">Déconnexion</button>
                        </form>
                    </div>
                </li>
            @else
                <li><a href="{{ route('login') }}" class="nav-btn">Connexion</a></li>
                <li><a href="{{ route('register') }}" class="nav-btn"
                        style="background: rgba(255, 255, 255, 0.2); border: 2px solid white; color: white;">S'inscrire</a>
                </li>
            @endauth
        </ul>
    </div>
